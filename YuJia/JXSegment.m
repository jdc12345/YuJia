...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _divideLineView.backgroundColor = [UIColor whiteColor];//[UIColor groupTableViewBackgroundColor];
        [_scrollView addSubview:_divideLineView];
        
        _divideView  = [[UIView alloc] init];
        _divideView.backgroundColor = [UIColor colorWithHexString:@"00bfff"];
...
- (void)updateChannels:(NSArray*)array{
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        [button setTitleColor:[UIColor colorWithHexString:@"00bfff"] forState:UIControlStateSelected];
