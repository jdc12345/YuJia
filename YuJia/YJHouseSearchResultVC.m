...
@interface YJHouseSearchResultVC ()<UITableViewDelegate,UITableViewDataSource,AMapSearchDelegate>
...
//    self.navigationController.navigationBar.translucent = false;
    ...
    [searchBtn setTitle:@"请输入小区名" forState:UIControlStateNormal];
...
- (void)choiceCondition:(UIButton*)sender{
    if (![self.residentialQuarters isEqualToString:@"0"]) {
...
-(void)setModel:(YJSearchHouseDetailResultModel *)model{
    _model = model;
    //初始化搜索各个参数
    self.codeUpperLevel = model.codeUpperLevel;
    self.codeUpperTwo = model.codeUpperTwo;
    self.areaCode = model.areaCode;
    self.residentialQuarters = model.rname;
    self.apartmentLayout = @"0";
    self.direction = @"0";
    self.rent = 0;
    self.rentalTyoe = 0;
    ...
    dist.keywords = model.codeUpperTwo;//查询行政数据，所以传citycode
