...
@interface EquipmentSettingViewController ()<UITableViewDataSource,UITableViewDelegate,UIPickerViewDelegate,UIPickerViewDataSource,UITextFieldDelegate>
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    sectionTitleLabel.text =  mName[[self.eqipmentModel.iconId integerValue]];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    return self.sightModel.equipmentList.count;
...
- (UIView *)customFootView{
    UIView *footView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kScreenW, 74 +93 +30)];
    footView.backgroundColor = [UIColor colorWithHexString:@"f1f1f1"];
    
    UIImageView *imageV = [[UIImageView alloc]initWithImage:[UIImage imageNamed:mIcon[[self.eqipmentModel.iconId integerValue]]]];
    imageV.backgroundColor = [UIColor whiteColor];
    [footView addSubview:imageV];
    [imageV mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(footView).with.offset(0);
    ...
    [sureBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(footView).with.offset(30 +93 +30);
...
- (void)httpRequestInfo{
    if (self.sightNameF.text.length >0) {
        self.eqipmentModel.name = self.sightNameF.text;
    }
    
    NSMutableDictionary *praraDict = [[NSMutableDictionary alloc]initWithDictionary:[self.eqipmentModel properties_aps]];
    [praraDict setValue:mDefineToken forKey:@"token"];
    
    [[HttpClient defaultClient]requestWithPath:[NSString stringWithFormat:@"%@",mAddEquipment] method:1 parameters:praraDict prepareExecute:^{
